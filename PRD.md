...
## 13. Contact attribution + row field updates (plain-mention, writes master-prospects directly)

A plain `@bot` mention attributing a contact/generic inbox to a company **already in `master-prospects`**, and/or changing any other field on that row — e.g. `"Aritzia — Jane Doe, VP Marketing, jane@aritzia.ca"`, `"generic inbox for Aritzia is service@aritzia.ca"`, or (2026-07-08) `"New Ventures BC — Angie Schick, Executive Director, https://linkedin.com/in/angieschick. Make this a warm pathway."`. Unlike chat (§12), this DOES write — but only through structurally-scoped functions, the second sanctioned exception to "never write master-prospects outside the Promotion Agent" (golden rule #1), and a deliberate carve-out to golden rule #2's "Contact fields... off-limits to every tool."

- **Routing:** `router.ts`'s `detectRoute` — checked before URL-forwarding. An email address anywhere in the text routes to `{kind:"contact"}` unconditionally; a LinkedIn profile URL (`linkedin.com/in/`) only counts when accompanied by other text, so a bare pasted LinkedIn URL still forwards to n8n exactly as before. **Known limit:** the route still requires an email or LinkedIn URL to trigger — a follow-up message with only a field change ("actually make that a warm lead too", no contact info) won't currently route here on its own.
- **Parsing:** `src/paths/contact.ts`'s `parseContactMessage` — one Anthropic call, no D12 substring check (the user is typing directly, not extracting from a fetched document). Returns the org-name guess, one or more contacts (name/title/email/linkedin, or a generic-inbox flag), a Why Them addition (ONLY when the message explicitly frames something as a reason to approach the org), a Notes addition (everything else), and (2026-07-08) `fieldUpdates` — explicit `{field, value}` pairs for any other column the message clearly asks to change, `field` constrained to exactly one of `MASTER_FIELD_KEYS` (`src/types.ts`) so the model can't invent a column name.
...
- **Writing:** two structurally-scoped `src/sheets.ts` functions, each with its own fixed key->column map, neither able to touch a column outside its own map: `updateMasterContactFields` — Primary Contact Name/Title/Email/LinkedIn (N/O/P/Q), Secondary Contact Name/Title/LinkedIn/Email (R/S/T/U), Generic Intake Email (V), plus append-only Why Them (F, via `appendAggregate`) and Notes (AH). `updateMasterFields` (added 2026-07-08, scope decided by Tej: "truly everything, including Category/Source Type/Org Name" over a narrower human-owned-columns-only option) — every other column via `MASTER_FIELD_KEYS`: Prospect ID, Organization Name, Category, Subsector, HQ/Geography, Potential Mutual Value, Programming Angle, Source Type, Source Link, Warm Lead?/Person/Path, Stage, Last Touch Date/Channel, Next Step, Next Follow-up Date, Owner, Funding Type, Estimated Capacity, Target Ask Range, Exclusivity Play?, Budget Window. Category and Source Type are validated against their live enums before writing (golden rule #15 — Sheets' own strict validation doesn't protect API writes); Warm Lead? is coerced to a real boolean (`coerceBoolean`) for its checkbox column (e.g. "make this a warm pathway" → `true`). Each field update is applied independently, so one invalid value (e.g. a bad Category) doesn't block the rest of the message's changes — failures are reported back in-thread rather than silently dropped or failing the whole update.
  - **Accepted risk (2026-07-08):** this is now a second, independent write path onto columns the Promotion Agent also writes (B/C/D/I/J) — there's no locking between them, so a contact-attribution edit racing a live `promote` run on the same row is possible. Not guarded against in code; noted here per AGENTS.md golden rule #1.
- **Primary vs. secondary:** `src/contact/contactSlot.ts`'s `decideContactSlot` (pure, unit-tested) — primary if N/O/P/Q are all blank, else secondary if R/S/T/U are all blank, else ambiguous (asks Tej whether to overwrite primary, overwrite secondary, or just log in Notes). (2026-07-08: Tej added Secondary Contact Title/Email columns, so secondary now has full parity with primary — no more Notes overflow for secondary title/email.)
