...
1. **Never write to `master-prospects` outside the Promotion Agent (PRD §11 / PLAN Stage 7) or the contact-attribution feature (PRD §13, golden rule #2's carve-out).** Everywhere else (all 4 file/URL intake paths), append to `data-staging` only — unchanged, still absolute. Within the Promotion Agent, the agent has real discretion over *when and what* to write, driven by context rather than a fixed external script — but every write **tool** it can call is itself structurally restricted in code, not just prompted: `append_master_row` can only ever populate the same 9 columns (PRD §10.5), `update_master_aggregate_row` can only ever touch Why Them + Source Link. Discretion is over sequencing and judgment; the tools themselves define the hard boundary (defense in depth, matching golden rule #15). **Note (2026-07-08):** the contact-attribution feature's `updateMasterFields` can now also touch B/C/D/I/J (Organization Name/Category/Subsector/Source Type/Source Link) — columns that used to be exclusively Promotion-Agent territory — per Tej's explicit "truly everything" scope decision. Two independent write paths can now touch the same cell; there's no locking between them, so a contact-attribution edit and a live `promote` run racing on the same row is a known, accepted risk, not something the code guards against.
2. **Contact fields are opportunistic, never a research target (revised 2026-07-04).** The agent must not go looking for a contact name/email as its own task — filling Contact isn't required for a row to be considered done. But if a name surfaces naturally with strong confidence while researching something else, it should be flagged to Tej in Slack (via `ask_tej_on_slack` or a note in the run summary), never written directly into either sheet. Writing an actual Contact value anywhere is still off-limits to every tool — **except the one narrow, deliberate carve-out below (2026-07-07, expanded 2026-07-08).**
   - **Carve-out: contact attribution + row field updates via a plain-text Slack mention (PRD §13).** Tej can @mention the bot with just text (no file/URL) — e.g. `"Aritzia — Jane Doe, VP Marketing, jane@aritzia.ca. Make this a warm pathway."` — to attribute a contact/generic inbox AND/OR change any other field on a company already in `master-prospects`. Two structurally-scoped functions do the writing, both in `src/sheets.ts`: `updateMasterContactFields` — Primary Contact Name/Title/Email/LinkedIn (N/O/P/Q), Secondary Contact Name/Title/LinkedIn/Email (R/S/T/U), Generic Intake Email (V), plus append-only Why Them (F) and Notes (AH); and `updateMasterFields` (added 2026-07-08, "familiar with any of the columns") — every other column via an explicit `MasterFieldKey` -> column map (`src/types.ts`'s `MASTER_FIELD_KEYS`), covering Prospect ID/Organization Name/Category/Subsector/HQ/Potential Mutual Value/Programming Angle/Source Type/Source Link/Warm Lead?/Warm Lead Person/Warm Lead Path/Stage/Last Touch Date/Last Touch Channel/Next Step/Next Follow-up Date/Owner/Funding Type/Estimated Capacity/Target Ask Range/Exclusivity Play?/Budget Window. Category and Source Type are validated against their live enums before writing (golden rule #15); Warm Lead? is coerced to a real boolean (`coerceBoolean`) for its checkbox column. Neither function can touch a column outside its own map regardless of input. Full design: `src/contact/` (`matchOrg.ts`, `contactSlot.ts`, `runContactAgent.ts`), `src/paths/contact.ts`, PLAN.md's 2026-07-07/2026-07-08 Execution Log entries.
...
- `master-prospects` columns A–AH (header row 2, data from row 3; confirmed live 2026-07-08 after Tej added Secondary Contact Title (S) + Secondary Contact Email (U), which shifted every column from the old S onward right by two): `A Prospect ID · B Organization Name · C Category · D Subsector · E HQ / Geography · F Why Them · G Potential Mutual Value · H Programming Angle · I Source Type · J Source Link · K Warm Lead? · L Warm Lead Person · M Warm Lead Path · N Primary Contact Name · O Title · P Email · Q LinkedIn URL · R Secondary Contact Name · S Secondary Contact Title · T Secondary Contact LinkedIn · U Secondary Contact Email · V Generic Intake Email · W Stage · X Last Touch Date · Y Last Touch Channel · Z Next Step · AA Next Follow-up Date · AB Owner · AC Funding Type · AD Estimated Capacity · AE Target Ask Range · AF Exclusivity Play? (Y/N/Unknown) · AG Budget Window · AH Notes`. The Promotion feature (PRD §10) only ever writes B/C/D/F/I/J/K/M/AH on append, and only F/J on aggregation (§10.4) — see PLAN Stage 6. The contact-attribution feature (§13) writes N–V — see `updateMasterContactFields` in `src/sheets.ts`.
