...
2. **Contact fields are opportunistic, never a research target (revised 2026-07-04).** The agent must not go looking for a contact name/email as its own task — filling Contact isn't required for a row to be considered done. But if a name surfaces naturally with strong confidence while researching something else, it should be flagged to Tej in Slack (via `ask_tej_on_slack` or a note in the run summary), never written directly into either sheet. Writing an actual Contact value anywhere is still off-limits to every tool — **except the one narrow, deliberate carve-out below (2026-07-07).**
   - **Carve-out: contact attribution via a plain-text Slack mention (PRD §13).** Tej can @mention the bot with just text (no file/URL) — e.g. `"Aritzia — Jane Doe, VP Marketing, jane@aritzia.ca"` — to attribute a contact or generic inbox to a company already in `master-prospects`. The ONLY function allowed to write these columns is `updateMasterContactFields` (`src/sheets.ts`) — structurally restricted, like `append_master_row`/`update_master_aggregate_row`, to an explicit column map: Primary Contact Name/Title/Email/LinkedIn (N/O/P/Q), Secondary Contact Name/LinkedIn (R/S), Generic Intake Email (T), plus append-only Why Them (F) and Notes (AF). It cannot touch anything else regardless of input — same defense-in-depth as golden rule #1/#15. Secondary has no Title/Email column; if given, that overflow goes to Notes. Full design: `src/contact/` (`matchOrg.ts`, `contactSlot.ts`, `runContactAgent.ts`), `src/paths/contact.ts`, PLAN.md's 2026-07-07 Execution Log entry.
